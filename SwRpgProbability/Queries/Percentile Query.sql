SELECT
  ...
  FROM [SwRpgProbability].[dbo].[PoolCombination]
	LEFT JOIN [SwRpgProbability].[dbo].[Pool] AS [PositivePool] ON ( [PoolCombination].[PositivePoolId] = [PositivePool].[PoolId])
	LEFT JOIN [SwRpgProbability].[dbo].[Pool] AS [NegativePool] ON ( [PoolCombination].[NegativePoolId] = [NegativePool].[PoolId])
