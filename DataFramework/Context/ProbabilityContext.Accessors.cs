...
namespace DataFramework.Context
{
	public static class ProbabilityContextAccessors
	{
		...
		public static Die GetDie(this ProbabilityContext context, string die) =>
			context.Dice.Where(w => w.Name == die.ToString())
		...
		public static PoolCombination GetPoolCombination(this ProbabilityContext context, int positiveId, int negativeId) => context.PoolCombinations.Where(w => w.PositivePoolId == positiveId && w.NegativePoolId == negativeId)
